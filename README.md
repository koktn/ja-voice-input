...
2. **Command キーを 2 回**(左右どちらでも。変更可)押す → ポン♪ と鳴ったら話す
3. 話し終えて約 0.8 秒無音になると自動で認識・整形され、カーソル位置に貼り付けられる
4. 長考したいときは録音中にもう一度ホットキーを押すと即座に打ち切って変換に進む

録音開始後に別のアプリ・ウィンドウ・入力欄へ移動した場合、誤入力を防ぐため自動貼り付けは中止されます。認識結果はクリップボードに残り、通知から確認できます。`paste.cancel_on_focus_change: false` で従来の「完了時の入力先へ貼り付ける」動作にも戻せます。
...
常駐起動時には STT と Ollama のモデルをバックグラウンドでウォームアップします。`llm.mode: auto` にすると、フィラーを検出した入力だけ LLM へ送り、それ以外は辞書置換と簡易整形ですぐに確定します。各セッション後の INFO ログには録音・STT・後処理・貼り付けの所要時間だけが出力され、音声入力の本文は `-v` 指定時のみ表示されます。

クリップボード復元では画像・ファイル・リッチテキストを含む全形式を保存します。貼り付け待機中にユーザーが新しくコピーした場合は、その内容を上書きしません。
...
├── paste.py        # 入力先検証 + Cmd+V(全形式のクリップボード復元付き)
└── feedback.py     # 効果音・通知
```
